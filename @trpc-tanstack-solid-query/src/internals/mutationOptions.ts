...
> = DistributiveOmit<
  MutationOptions<TOutput, TError, TInput, TContext>,
  ReservedOptions
> &
  TRPCQueryBaseOptions &
...
interface TRPCMutationOptionsOut<
  ...
  TContext,
  TFeatureFlags extends FeatureFlags,
> extends MutationOptions<TOutput, TError, TInput, TContext>,
    TRPCQueryOptionsResult {
  ...
  return {
    ...
    onSuccess(...args: any[]) {
      const originalFn = (): MaybePromise<void> =>
        (opts?.onSuccess?.apply(undefined, args as any) ??
          defaultOpts?.onSuccess?.apply(undefined, args as any)) as MaybePromise<void>;
